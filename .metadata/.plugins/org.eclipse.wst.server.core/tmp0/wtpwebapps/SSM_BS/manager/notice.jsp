...
<body
	...
	<!-- BEGIN CONTENT -->
										...
										<c:if test="${notice.type=='0'}"><label style="color:green;">管理者公告</label></c:if>
                                          <c:if test="${notice.type=='1'}"><label style="color:red;">优惠信息</label></c:if>
														...
														<td>
																	...
																	<a class="btn btn-circle btn-icon-only btn-default tooltips"  href='javascript:void(0);' onclick="javascript:showAlertModel('警告信息', '确认删除？','<c:url value="/notice/deleteById?id=${notice.id}"></c:url>');" data-toggle="tooltip" data-placement="bottom" data-html="true" data-original-title="删除公告">
			...
			<!-- END CONTAINER -->
		<!-- 正文部分结束 -->
		<input type="hidden" id="deleted" value="${noticeDeleted}"/>
	</div>
