...
public class BookController {
	private final String JSON_JSP = "/commons/jsondata.jsp";
	...
	public String bookSave(HttpServletRequest request, Model model, Book book,
			String[] readFreeName, HttpSession session) {
	...
	public String bookUpdate(HttpServletRequest request, Model model,
			Book book, String[] readFreeName, String[] readFreeBookId,
			String[] readFreeId, String[] readFreeNumber, HttpSession session) {
		int xxx = bookService.updateByPrimaryKeySelective(book);
		if (readFreeName.length > 1 && xxx > 0) {
			System.out.println("总共有几条：" + readFreeName.length);
			List<ReadFree> readList=readFreeService.selectByBookId(book.getId());
			for (int j = 0; j < readFreeName.length - 1; j++) {
				if (Integer.parseInt(readFreeId[j]) != 0) {
						ReadFree readfree = new ReadFree();
						readfree.setBookId(book.getId());
					...
					ReadFree readfree = new ReadFree();
					readfree.setBookId(book.getId());
					readfree.setNumber(Integer.parseInt(readFreeNumber[j])+1);
					readfree.setSrc(readFreeName[j]);
					readFreeService.saveOrUpdate(readfree);
				}
			}
	...
	@RequestMapping("/readFreeDelete")
	public String readFreeDelete(HttpServletRequest request, Model model,
			ReadFree readFree, HttpSession session) {
		int id=Integer.parseInt(request.getParameter("id"));
		readFreeService.deleteById(id);
		return "JSON_JSP";
	}

	@RequestMapping("/showBySubType")
	public String showBySubType(HttpServletRequest request, Model model,
			Book book, HttpSession session) {
		Map<String, Object> condition = new HashMap<String, Object>();
	...
	public String search(HttpServletRequest request, Model model, Book book,
			HttpSession session, String search) {
		Map<String, Object> condition = new HashMap<String, Object>();
		...
		if (null != cus) {
			List<Searchinfo> searchList = searchInfoService
					.selectAllByCusId(cus.getId());
	...
	public String showBookDetial(HttpServletRequest request, Model model,
			Book book, HttpSession session) {
		...
		// 获取免费试读
		List<ReadFree> readfreeList = readFreeService.selectByBookId(book
				.getId());
		...
		if (suggestBookList.size() <= 4) {
			...
			int[] m = CustomerController.randomCommon(0,
					suggestBookList.size(), 4);
			for (int i : m) {
		...
		if (null != session.getAttribute("shopCarMessage")) {
			model.addAttribute("shopCarMessage",
					session.getAttribute("shopCarMessage"));
	...
	public String addShopCar(HttpServletRequest request, Model model,
			ShopCar shopCar, HttpSession session) {
		Customer cus = (Customer) session.getAttribute("loginCustomer");
		if (null == cus) {
			session.setAttribute("returnUrl", "/book/showBookDetial?id="
					+ shopCar.getBookid());
			...
			List<ShopCar> shopCarList = shopCarService.selectByCusId(cus
					.getId());
			session.setAttribute("shopCarCount", shopCarList.size());
			shopCar.setCustomerid(cus.getId());
			int xx = shopCarService.insert(shopCar);
			if (xx > 0) {
				session.setAttribute("shopCarMessage",
						"加入购物车成功，您可以通过点击右上角购物车进行结算哦！");
			} else {
				session.setAttribute("shopCarMessage", "加入购物车失败，请重试！");
			}
			List<ShopCar> shopCarList1 = shopCarService.selectByCusId(cus
					.getId());
	 ...
	 */
	@RequestMapping("/buy")
	public String buy(HttpServletRequest request, Model model, ShopCar shopCar,
			HttpSession session) {
		Customer cus = (Customer) session.getAttribute("loginCustomer");
		if (null == cus) {
			session.setAttribute("returnUrl", "/book/showBookDetial?id="
			...
			session.setAttribute("type", type);
			shopCar.setCustomerid(cus.getId());
			BigDecimal xx = new BigDecimal(shopCar.getPrice()
					* shopCar.getDiscount() / 10);
			double discountMoney = xx.setScale(2, BigDecimal.ROUND_HALF_UP)
					.doubleValue();
			...
			double totalMoney = shopCar.getQuantity()
					* shopCar.getDiscountedPrice();
