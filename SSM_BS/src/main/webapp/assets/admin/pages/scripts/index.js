var Index = function () {
    return {
                ...
                $("#site_activities").bind("plothover", function (event, pos, item) {
                    ...
                    if (item) {
                        if (previousPoint2 != item.dataIndex) {
                            ...
                            var x = item.datapoint[0].toFixed(2),
                                y = item.datapoint[1].toFixed(2);
                            showChartTooltip(item.pageX, item.pageY, item.datapoint[0], item.datapoint[0] + "时交易额：" + item.datapoint[1] + '元');
                        }
                    }
                });
		...
		initDrawSummaryChart: function () {			
			...
			data.push(monthlyInvestAmountData);
			CHART_HELPER.drawBarChart($('#summary')[0], data, legend, xAxis);
		},
		
		initDrawPlatformMarginChart: function () {							
